...
const buscarProductosPoteste = async (req, res) => {
  const { query } = req.params;

  try {
    const productos = await Producto.find({
      $or: [
        { nombreproductos: { $regex: query, $options: "i" } },
...
const cambiarEstadoFavorito = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    // Si la propiedad 'favoritos' no existe, establecerla en true por defecto
    if (producto.favoritos === undefined) {
      producto.favoritos = true;
    } else {
    ...
    // Mensaje de respuesta según el nuevo estado
    const mensaje = producto.favoritos
      ? "Se ha convertido en uno de tus productos favoritos."
      : "Dejó de ser tu producto favorito.";

    res.status(200).json({ message: mensaje, producto });
  } catch (error) {
